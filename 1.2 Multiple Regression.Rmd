...
library(contrast) # not too important. #in R consolde use install.packages("contrast") to install package contrast and its dependencies
library(tidyverse) # package of packages: ggplot2, dplyr, readr...
```
...
* ISLR, Chapter 3
* Stat Sleuth: Ch 9-10 Multiple Reg, Ch 5 One Way ANOVA
...
|Displacement|Volume displaced by the cylinders|
...
It is always crucial to look at the data first. For the purpose of shortening the lecture time, we put this part in Appendix 1. There the original data "Car_04.csv" are cleaned and a new version called "car_04_regular.csv" is written out. 
...
data1 <- read.csv("car_04_regular.csv", header=TRUE) #View(data1)
```
...
**Interpreation of $\beta_1$** is that in general, the mean $y$ will change by $\beta_1$ if a car is 1" longer. 
...
**Interpreation of $\beta_1$** is that in general, the mean $y$ will change by $\beta_1$ if a car is 1" longer and the 'HorsePower' remains the same. 
...
**Question**: Are all the $\beta_1$s the same in the 3 models above? 
...
let $X$ denote a $n \times (p+1)$ matrix whose $(i,j)$th element is $x_{ij}$. That is:
...
1. $\hat\beta_{Length}$ = `r coef(fit3)[2]` for Model 3. Does this make sense?
...
Given a fixed model, assuming all the linear model assumptions are correct, then we can make the following inferences:
...
Use Model 3 and `fit3` as an example. What does each $t$-test do? 
...
**Theorem**: Assuming the linear model assumptions hold, then under the $H_0$,
$$F_{\text{stat}}:=\frac{\frac{RSS(H_0)-RSS(H_1)}{df_1}}{\frac{RSS(H_1)}{df_2}} \sim F_{df_1, df_2}$$
where 

$${df}_1= \text{number of parameters in } H_1 - \text{number of parameters in } H_0,$$ 
and 

$${df}_2= \text{number of observations} - \text{number of parameters in } H_1.$$
...
```{r}
pf(1.9373, 2, 219, lower.tail=F)  # hist(rf(10000, 2, 219), breaks=40)
```


**Use `anova()` ** to carry out the above F-test `anova(H_0, H_1)`.
...
Based on Model 3, the mean of `MPG_City` among all cars of length=180, HP=225, width=80, seating=5, cylinders=4, displacement=3.5, transmission="automatic", continent="Am" is
...
What do you think about the linearity, homoscedasticity and normality assumptions?
...
Model with one categorical variable is termed as a "One Way ANOVA"

First, we get the sample means and sample standard deviations for each group, and plot them
```{r  eval = T}
data1 %>%
group_by(Continent) %>%
  summarise(
...
Then the above ANOVA model is the same as

$$y_i = \beta_{Am} + \beta_{As} x_{1,i} + \beta_E x_{2,i} + \epsilon_i = \beta_{Am} + \beta_{As} I(Continent = As) + \beta_E I(Continent = E) + \epsilon_i.$$

To test $$H_0: \mu_{Am} = \mu_{As} = \mu_{E}$$ is the same as to test
$$H_0: \beta_{As} = \beta_{E} = 0.$$

We can use `lm()` to fit the model and carry out the tests.
...
We can choose the base level according to what we want. We demonstrate this by setting up Asian cars as the base category:
...
We also show a model without the intercept $\beta_0$
```{r}
...
**Drawback of One-way ANOVA:** Other factors should also be taken into consideration.

### Model 2 - Model without Interaction

Let us consider another important variable `Horsepower` in addtion to `Continent`. 
...
We fail to reject $H_0$ at $\alpha=0.01$, i.e., controlling HP, we do not have enough evidence to say that the effect of `Continent` is different.


**Remark:** Models without interactions are simple, and we can test **Continent** effects directly. But we may ask ourselves: is this assumtion of no interaction reasonable?


### Model 3 - Model with Interaction

It is possible that the effect of `HP` depends on the levels of `Continent`.
...
As we can see from the plot, Asian cars with smaller HP are more effiecient; while European cars with larger HP are more efficient. 

Hence, we introduce the following models with interactions.
...
To test whether there is no interaction is the same as to test
...
We do reject the null hypothesis with a $p$-value of $.0013$, not terribly small. That indicates some interaction effect between HP and Continent.

So the final conclusion would be

- Asian cars with smaller HP are more effiecient; 
- European cars with larger HP are more efficient.

We skip the detailed investigations. 
...
2. Take a look at `Transmission`. There are predominately automatic cars. Perhaps we should only include automatic cars and change our population. 
```{r}
table(data2$Transmission)
```
3. What would have happened if you have added Make.Model into the model???

**Model Diagnostic**: check if the three assumptions of linear models are met.

Residuals vs Fitted and qqnormal Plot:
```{r  eval = T}
par(mfrow=c(1,2))
plot(fit.all, 1) 
plot(fit.all, 2) 
